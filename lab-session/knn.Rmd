...
tp<-sum(test.Y[which(pred.Y=='No')]=='No')
fp<-sum(test.Y[which(pred.Y=='No')]=='Yes')
prec<-tp/(tp+fp)
recall<-tp/sum(test.Y=='No')
```
...
F1<-function(pred,ref){
  tp<-sum(ref[which(pred=='No')]=='No')
  fp<-sum(ref[which(pred=='No')]=='Yes')
  prec<-tp/(tp+fp)
  recall<-tp/sum(ref=='No')
